...
from tensorflow.python.keras.layers import RNN, AbstractRNNCell, Layer
...
class ConvARMACell(AbstractRNNCell):
    ...
    def output_size(self) -> TensorShape:
        return (
            TensorShape((self.q, *self.image_shape[1:-1], self.units))
            if self.return_lags
            else TensorShape((*self.image_shape[:-1], self.units))
        )

    def build(self, input_shape: Tuple[int, ...]) -> None:
        input_dim = input_shape[-1]  # channels last
        self.p = input_shape[-4]  # type: ignore
        ...
        self.ar_parameters = self.add_weight(
            shape=(self.units, self.p), initializer="glorot_uniform", name="ar"
        )
        self.ma_parameters = self.add_weight(
            shape=(self.units, self.q), initializer="glorot_uniform", name="ma"
        )

        if self.use_bias:
            self.arma_bias = self.add_weight(
                shape=(self.units,), initializer="zeros", name="arma_bias"
            )
            self.ar_conv_bias = self.add_weight(
                shape=(self.p, self.units), name="ar_conv_bias", initializer="zeros"
            )
    ...
    def call(self, inputs: tf.Tensor, states: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        for i, split in enumerate(splits_ar):
            split = split[:, 0, :, :, :]
            conv_out = self.input_conv(
                split, self.kernel[:, :, :, i, :], self.ar_conv_bias[i, :]  # type: ignore
            )
        ...
        for i, split in enumerate(splits_ma):
            split = split[:, 0, :, :, :]
            conv_out = self.input_conv(
                split, self.recurrent_kernel[:, :, :, i, :], self.ma_conv_bias[i, :]  # type: ignore
            )
        ...
        output_states = tf.concat(
            [tf.expand_dims(output, axis=1), input_states[:, :-1, :, :, :]], axis=1
        )
    ...
    def input_conv(
        ...
        conv_out = backend.conv2d(
            x, w, strides=(1, 1), padding=padding, data_format="channels_last"
        )
        if b is not None:
...
class SpatialDiffs(Layer):
    ...
    def call(self, inputs: tf.Tensor, *args: Any, **kwargs: Any) -> tf.Tensor:
        ...
        diff_1 = tf.concat(
            [diff_1[:, :, :, :shift_val] * 0, diff_1[:, :, :, shift_val:]], axis=-3
        )

        diff_2 = tf.roll(inputs, -shift_val, axis=-3) - inputs
        diff_2 = tf.concat(
            [diff_2[:, :, :, :shift_val], diff_2[:, :, :, shift_val:] * 0], axis=-3
        )

        diff_3 = tf.roll(inputs, shift_val, axis=-2) - inputs
        diff_3 = tf.concat(
            [diff_3[:, :, :, :, :shift_val] * 0, diff_3[:, :, :, :, shift_val:]],
            axis=-2,
        )

        diff_4 = tf.roll(inputs, -shift_val, axis=-2) - inputs
        diff_4 = tf.concat(
            [diff_4[:, :, :, :, :shift_val], diff_4[:, :, :, :, shift_val:] * 0],
            axis=-2,
        )
